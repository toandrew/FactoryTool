...
public final class MidiDeviceConnectionWatcher {
    ...
    final Queue<UsbDevice> deviceGrantQueue = new LinkedList<UsbDevice>();
    final HashSet<UsbDevice> grantedDevices = new HashSet<UsbDevice>();
     ...
     */
    private void onDeviceDetached( UsbDevice detachedDevice) {
	...
	private final class UsbMidiGrantedReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context receiverContext, Intent intent) {
			...
			if (USB_PERMISSION_GRANTED_ACTION.equals(action)) {
				...
				Log.d(Constants.TAG, "UsbMidiGrantedReceiver: granted[" + granted + "]thread.stopFlag[" + thread.stopFlag + "]");

				if (granted && !thread.stopFlag) {
                    grantedDevices.add(device);

                    UsbDeviceConnection deviceConnection = usbManager.openDevice(device);
                    if (deviceConnection == null) {
                        // Recover for next usb plugin/plugout events.
                        isGranting = false;
                        grantingDevice = null;
	...
	private final class MidiDeviceConnectionWatchThread extends Thread {
		 ...
		 */
		private synchronized void checkConnectedDevices() {
