 ...
 * @author K.Shoji
 */
public final class MidiOutputDevice extends MidiDeviceBase {
    private static final int BUFFER_POOL_SIZE = 1024;
    private final WaiterThread waiterThread;
	final LinkedList<byte[]> bufferPool = new LinkedList<byte[]>();
	...
	public MidiOutputDevice( UsbDevice usbDevice,  UsbDeviceConnection usbDeviceConnection,  UsbInterface usbInterface,  UsbEndpoint usbEndpoint) {
        ...
        waiterThread.setName("[MidiOutDevice waiterThread " + usbDevice.getDeviceName() + "]");
        this.start();
                        ...
                        for (bufferPosition = 0; bufferPosition < dequedDataBufferLength; bufferPosition += maxPacketSize) {
                            ...
                            while (true) {
                                if (bufferPosition > 0) {
                                    ...
                                    bytesWritten = usbDeviceConnection.bulkTransfer(usbEndpoint, endpointBuffer, endpointBufferLength, 10);
                                } else {
                                    // it's the first fragment.. copy is not required
                                    bytesWritten = usbDeviceConnection.bulkTransfer(usbEndpoint, dequedDataBuffer, endpointBufferLength, 10);
                                }
